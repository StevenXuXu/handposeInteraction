...
from hand_track.angle_util import pose_to_angles
from hand_track.draw_util import draw_hand_line, draw_hand_point
...
def main(choice):
    if choice == 1:
        ...
        with torch.no_grad():
            idx = 0
            data_list = []
            for dir in os.listdir(ops.test_path):
                # if '.jpg' not in file:
                #     continue
                file_path = os.path.join(ops.test_path, dir)
                name = dir
                print('name: ' + name)
                for file in os.listdir(file_path):
                    ...
                    img = cv2.imdecode(np.fromfile(os.path.join(file_path, file), dtype=np.uint8), -1)
...
if __name__ == "__main__":
    ...
    # 模型路径
    parser.add_argument('--model_path', type=str, default='inference/weights/pose_weight/resnet50_2021-418.pth',
                        help='model_path')
    # GPU选择
