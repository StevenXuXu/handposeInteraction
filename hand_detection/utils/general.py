...
from hand_detection.utils.google_utils import gsutil_getsize
from hand_detection.utils.metrics import fitness
from hand_detection.utils.torch_utils import init_torch_seeds
...
def get_latest_run(search_dir='.'):  # 指定目录
    # Return path to most recent 'last.pt' in /runs (i.e. to --resume from)
    last_list = glob.glob(f'{search_dir}/**/last*.pt', recursive=True)  # 在目录下查询
    return max(last_list, key=os.path.getctime) if last_list else ''  # 返回最后创建的文件
...
def non_max_suppression(prediction, conf_thres=0.25, iou_thres=0.45, classes=None, agnostic=False, multi_label=False,
    ...
    # Settings
    min_wh, max_wh = 100, 4096  # 2 (pixels) minimum and maximum box width and height
    max_det = 2  # 300 maximum number of detections per image
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        x[((x[..., 2] < min_wh) & (x[..., 3] < min_wh) | (x[..., 2] > max_wh) & (x[..., 3] > max_wh)), 4] = 0
